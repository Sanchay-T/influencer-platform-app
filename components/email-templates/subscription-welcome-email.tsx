  ...
  <Html>
    ...
    <Body style={main}>
      ...
      <Container style={container}>
        <Text style={title}>
          {`You're live on the ${planName} plan!`}
        </Text>

        <Text style={paragraph}>
          Hi{fullName ? ` ${fullName}` : ''}{businessName ? ` from ${businessName}` : ''},
        </Text>

        <Text style={paragraph}>
          Thanks for trusting Gemz with your creator workflow. Your subscription is active and the full {planName} toolkit is ready whenever you are.
        </Text>

        {planFeatures.length > 0 && (
          <Section style={featureSection}>
            <Text style={featureHeadline}>
              What you unlocked with the {planName} plan:
            </Text>
            <ul style={featureList}>
              {planFeatures.map((feature, idx) => (
                <li key={idx} style={featureItem}>{feature}</li>
              ))}
            </ul>
          </Section>
        )}
        ...
        <Text style={paragraph}>
          Need a strategy check-in or have feedback? Just reply directly to this email and our team will jump in.
        </Text>
