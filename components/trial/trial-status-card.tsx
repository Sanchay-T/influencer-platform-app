...
import { type TrialData, useFormattedCountdown } from '@/lib/hooks/useTrialCountdown';
		...
		switch (trialData.status) {
			...
			case 'cancelled':
				return (
					<Badge
						variant="secondary"
						className="bg-zinc-800 text-zinc-300 border border-zinc-700/50"
					>
						Cancelled
					</Badge>
				);
			...
			default:
				return (
					<Badge variant="outline" className="text-zinc-300 border-zinc-700/50">
						Pending
					</Badge>
				);
		}
		...
		<Card className={`${className} bg-zinc-900/80 border border-zinc-700/50`}>
			...
			<CardContent className="space-y-6">
				...
				<div className="text-center">
					...
					<p className="text-sm text-zinc-400">{countdown.formatted.statusText}</p>
				</div>
				...
				<div className="space-y-2">
					...
					<Progress value={countdown.progressPercentage} className="h-2" />
				</div>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<p className="text-zinc-400">
								{trialData.startDate
									? new Date(trialData.startDate).toLocaleDateString('en-US', {
											month: 'short',
											day: 'numeric',
											year: 'numeric',
										})
					...
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<p className="text-zinc-400">
								{trialData.endDate
									? new Date(trialData.endDate).toLocaleDateString('en-US', {
											month: 'short',
											day: 'numeric',
											year: 'numeric',
										})
