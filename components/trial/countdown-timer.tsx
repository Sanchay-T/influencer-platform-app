...
import { useTrialCountdown, type TrialData } from '@/lib/hooks/useTrialCountdown';
  ...
  return (
    <div className={`${sizeClasses.container} ${className}`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className={`${sizeClasses.time} ${getStatusColor()}`}>
          ...
          <div className={`w-full bg-gray-200 rounded-full ${sizeClasses.progress}`}>
            <div 
              className={`${sizeClasses.progress} rounded-full transition-all duration-300 ${
                countdown.isExpired ? 'bg-red-500' : 
                countdown.daysRemaining <= 1 ? 'bg-orange-500' : 'bg-blue-500'
              }`}
