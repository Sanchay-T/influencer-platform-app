...
/**
 * Delete Specific Users: Ramon/Gemz and ram/test
 */
...
async function deleteRamGemzUsers() {
  ...
  try {
    ...
    for (const targetUser of targetUsers) {
      ...
      // Check campaigns
      const campaignsQuery = `SELECT COUNT(*) as count FROM campaigns WHERE "userId" = $1;`;
      ...
      // Check scraping jobs
      const jobsQuery = `SELECT COUNT(*) as count FROM scraping_jobs WHERE "userId" = $1;`;
      ...
      try {
        ...
        console.log(`   ✅ Deleted user: ${deletedUser.rows[0]?.name || user.name}`);
        ...
        console.error(`❌ Error deleting ${user.name}, rolled back:`, error.message);
      }
    }
  ...
  console.log('\n📊 SUMMARY:');
  console.log('• Deleted: Ramon / Gemz user');
  console.log('• Deleted: ram / test user');
  console.log('• Removed all associated campaigns and jobs');
