...
export async function POST(req: Request) {
    console.log('🚀 INICIO DE SOLICITUD POST A /api/scraping/tiktok');
    try {
        console.log('🔍 Paso 1: Verificando autenticación del usuario');
        ...
        try {
            ...
            user = data?.user;
            userError = error;
            console.log('🔑 Resultado de autenticación:', user ? 'Usuario autenticado' : 'Usuario no autenticado');
        } catch (authError: any) {
            ...
            if (authError.message.includes('Invalid UTF-8 sequence')) {
                console.log('🔄 Intentando limpiar la sesión debido a error de codificación UTF-8');
                try {
                    await supabase.auth.signOut();
                    console.log('✅ Sesión limpiada exitosamente');
                } catch (signOutError: any) {
                    console.error('❌ Error al limpiar la sesión:', signOutError);
                }
            }
        ...
        console.log('📝 Longitud del cuerpo de la solicitud:', bodyText.length);
        console.log('📝 Primeros 100 caracteres del cuerpo:', bodyText.substring(0, 100));
        
        console.log('🔍 Paso 3: Parseando JSON del cuerpo');
        // Intentar parsear el JSON con manejo de errores
        let body;
        try {
            body = JSON.parse(bodyText);
            console.log('✅ JSON parseado correctamente');
            console.log('📦 Estructura del cuerpo:', JSON.stringify(body, null, 2).substring(0, 200) + '...');
...
export async function GET(req: Request) {
    try {
        ...
        // Comprobar si el job está en procesamiento pero no se ha actualizado en los últimos 3 minutos
        // Esto indica que posiblemente QStash falló y necesitamos intervenir
        if ((job.status === 'processing' || job.status === 'pending') && 
            job.updatedAt && 
            (new Date().getTime() - new Date(job.updatedAt).getTime()) > 3 * 60 * 1000) {
            ...
            try {
                // Verificar si tenemos suficientes resultados para considerar el job como completado
                const totalResults = job.results?.reduce((total, result) => {
                    return total + (result.creators?.length || 0);
                }, 0) || 0;
                
                // Si tenemos 90% o más de los resultados objetivo, marcar como completado
                const completionThreshold = Math.floor(job.targetResults * 0.9);
                const hasEnoughResults = totalResults >= completionThreshold || (job.processedResults || 0) >= completionThreshold;
                
                if (hasEnoughResults) {
                    console.log('🎯 AUTOCOMPLETANDO JOB: Suficientes resultados detectados');
                    console.log('📊 Resultados actuales:', totalResults);
                    console.log('📊 Objetivo:', job.targetResults);
                    console.log('📊 Umbral de completado:', completionThreshold);
                    ...
                    await db.update(scrapingJobs)
                        .set({ 
                            ...
                            updatedAt: new Date(),
                            progress: '100',
                            processedResults: Math.max(job.processedResults || 0, totalResults)
                        })
                        .where(eq(scrapingJobs.id, job.id));
                    
                    console.log('✅ Job auto-completado por tener suficientes resultados');
