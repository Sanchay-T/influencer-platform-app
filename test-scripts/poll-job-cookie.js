#!/usr/bin/env node
// Create a scraping job (optional) and poll until it completes using a Clerk __session cookie.
...
function printHelp() {
  console.log(`\nPoll Job (Cookie)\n\nUsage:\n  node test-scripts/poll-job-cookie.js --path /api/scraping/tiktok --cookie "__session=..." --campaign-id <id> --keywords '["iphone 17 pro"]' --target-results 100\n`)
}

async function sleep(ms) { return new Promise((r) => setTimeout(r, ms)) }

async function createJob(baseUrl, path, cookie, body) {
...
async function getStatus(baseUrl, path, cookie, jobId) {
...
async function main() {
  ...
  if (!args.cookie) { console.error('--cookie is required'); process.exit(1) }
  ...
  if (!jobId) {
    ...
    const createRes = await createJob(baseUrl, path, args.cookie, { keywords, targetResults, campaignId: args.campaignId })
  ...
  while (true) {
    ...
    let status
    try {
      status = await getStatus(baseUrl, path, args.cookie, jobId)
    } catch (e) {
...
main().catch((err) => { console.error('poll-job-cookie failed:', err); process.exit(1) })
