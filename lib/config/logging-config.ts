	...
	async getConfig(): Promise<LoggingConfig> {
		...
		try {
			...
			structuredConsole.warn(
				'[LOGGING-CONFIG] Failed to load from database, using defaults:',
				error
			);
	...
	private async loadConfigFromDatabase(
		...
		try {
			...
			config.sentryTracesSampleRate = await this.getConfigValue(
				...
				environment,
				defaultConfig.sentryTracesSampleRate
			);
			...
			config.sentrySessionSampleRate = await this.getConfigValue(
				...
				environment,
				defaultConfig.sentrySessionSampleRate
			);
			...
			config.slowOperationThreshold = await this.getConfigValue(
				...
				environment,
				defaultConfig.slowOperationThreshold
			);
			...
			config.rateLimitPerMinute = await this.getConfigValue(
				...
				environment,
				defaultConfig.rateLimitPerMinute
			);
		...
		try {
			...
			const configEntries = [
				...
				{ key: 'enable_file', value: defaultConfig.enableFile, type: 'boolean' },
				{
					...
					value: defaultConfig.sentryTracesSampleRate,
					type: 'number',
				},
				{
					key: 'sentry_session_sample_rate',
					value: defaultConfig.sentrySessionSampleRate,
					type: 'number',
				},
				{
					key: 'sentry_error_sample_rate',
					value: defaultConfig.sentryErrorSampleRate,
					type: 'number',
				},
				{
					key: 'enable_performance_tracking',
					value: defaultConfig.enablePerformanceTracking,
					type: 'boolean',
				},
				{
					key: 'slow_operation_threshold',
					value: defaultConfig.slowOperationThreshold,
					type: 'number',
				},
			...
			structuredConsole.log(
				`[LOGGING-CONFIG] Initialized default configurations for ${environment}`
			);
		} catch (error) {
