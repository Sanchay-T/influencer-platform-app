...
import { type SystemConfiguration, systemConfigurations } from '@/lib/db/schema';
	...
	static async get(category: string, key: string): Promise<any> {
		...
		try {
			...
			const config = await db.query.systemConfigurations.findFirst({
				where: and(eq(systemConfigurations.category, category), eq(systemConfigurations.key, key)),
			});

			if (config) {
				const validatedValue = validateValue(config.value, config.valueType);
				structuredConsole.log(
					`🔧 [CONFIG-DB] Found in database: ${cacheKey} = ${config.value} (type: ${config.valueType})`
				);
			...
			if (defaultConfig) {
				...
				structuredConsole.log(
					`🔧 [CONFIG-DEFAULT] Using default value: ${cacheKey} = ${defaultConfig.value}`
				);
				cache.set(cacheKey, validatedValue);
				return validatedValue;
			}
			...
			if (fallbackConfig) {
				...
				structuredConsole.log(
					`🔧 [CONFIG-DEFAULT] Using default value: ${cacheKey} = ${fallbackConfig.value}`
				);
				cache.set(cacheKey, validatedValue);
				return validatedValue;
			}
	...
	static async initializeDefaults(): Promise<void> {
		try {
			...
			for (const entry of defaultEntries) {
				await db.insert(systemConfigurations).values(entry).onConflictDoNothing();
			}
...
export const getQStashDelay = (type: string) => SystemConfig.get('qstash_delays', `${type}_delay`);

export const getJobTimeout = (type: string) => SystemConfig.get('timeouts', `${type}_timeout`);

export const getPollingInterval = (type: string) => SystemConfig.get('polling', `${type}_interval`);

export const getCacheDuration = (type: string) => SystemConfig.get('cache', `${type}_duration`);

export const getProcessingSetting = (type: string) => SystemConfig.get('processing', type);
