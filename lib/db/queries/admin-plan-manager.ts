...
export async function grantPlanToUserByEmail(
	email: string,
	planKey: string,
	options: PlanGrantOptions = {}
) {
	...
	await db.transaction(async (tx) => {
		...
		const hasSubscription = await tx
		...
		const hasUsage = await tx
			.select({ id: userUsage.id })
			.from(userUsage)
			.where(eq(userUsage.userId, user.id));

		if (hasUsage.length) {
			await tx.update(userUsage).set(usageUpdates).where(eq(userUsage.userId, user.id));
		} else {
		...
		if (billingUpdates.stripeCustomerId) {
			const hasBilling = await tx
				.select({ id: userBilling.id })
				.from(userBilling)
				.where(eq(userBilling.userId, user.id));

			if (hasBilling.length) {
				await tx.update(userBilling).set(billingUpdates).where(eq(userBilling.userId, user.id));
			} else {
