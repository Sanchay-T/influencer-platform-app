...
export function normalizeFeedItems({
	...
	keyword,
	limit,
}: NormalizeFeedItemsInput): NormalizedCreator[] {
	const generatedAt = feed.generatedAt;
	const items = Array.isArray(feed.items) ? feed.items : [];
	const sliceLimit =
		typeof limit === 'number' && limit > 0 ? Math.min(limit, items.length) : items.length;
	const selected = items.slice(0, sliceLimit);

	return selected.map((item, index) =>
		toNormalizedCreator(item, {
			keyword,
			generatedAt,
			ordinal: index,
...
export function toNormalizedCreator(
	item: FeedItem,
	...
	const engagementPercent =
		typeof item.creator.engagementPercent === 'number' ? item.creator.engagementPercent : null;
