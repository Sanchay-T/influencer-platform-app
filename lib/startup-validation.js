...
export function validateEnvironmentOnStartup() {
  const verbose = process.env.STARTUP_VALIDATION_VERBOSE === 'true' || process.env.NODE_ENV === 'production';
  ...
  try {
    ...
    if (!stripeValidation.isValid) {
      ...
      if (isProductionDeployment) {
        structuredConsole.error('[STARTUP-VALIDATION][ABORT] Stopping production deployment due to configuration errors');
    ...
    structuredConsole.error('[STARTUP-VALIDATION][CRITICAL] Critical validation error:', error.message);
