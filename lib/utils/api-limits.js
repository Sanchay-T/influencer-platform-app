 ...
 */
export function calculateApiCallLimit(
	targetResults,
	platform,
	searchType,
	...
	structuredConsole.log(
		`🔢 [API-LIMITS] Calculating for: ${platform}_${searchType}, target: ${targetResults}, mode: ${mode}`
	);
	...
	const rawKey = `${platform}_${searchType}`;
	const key = rawKey.toLowerCase();
	const efficiency = PLATFORM_EFFICIENCY[key];

	if (!efficiency) {
		structuredConsole.warn(`⚠️ [API-LIMITS] Unknown platform combination: ${rawKey}`);
	...
	structuredConsole.log(
		`🔢 [API-LIMITS] ${rawKey}: ${targetResults} creators → ${calculatedCalls} calculated → ${finalCalls} final (max: ${efficiency.maxCalls})`
	);
...
export function estimateResults(apiCalls, platform, searchType) {
	const key = `${platform}_${searchType}`.toLowerCase();
	const efficiency = PLATFORM_EFFICIENCY[key];
...
export function analyzeTargetFeasibility(targetResults, platform, searchType) {
	...
	return {
		...
		reason: achievable
			...
			: `Would require ${requiredCalls} calls (max: ${efficiency.maxCalls})`,
		recommendation: achievable
			? 'Proceed with calculated limits'
			: `Consider reducing target to ${maxAchievable} creators or using a different search type`,
	};
}
