...
export async function cleanTestSubscriptionsInProduction() {
  ...
  try {
    ...
    if (!hasBilling || !hasSubscriptions) {
      console.warn('[MIGRATION][WARN] Normalized billing tables missing; skipping automatic cleanup');
      return;
    }
    ...
    console.log(`[MIGRATION] FIND Found ${testSubs.length} test subscriptions to clean:`);
    ...
    if (!affectedUserIds.length) {
      console.warn('[MIGRATION][WARN] Test subscriptions detected but no valid user_ids were resolved; skipping automated cleanup.');
      return;
    }
    ...
    console.log(`[MIGRATION][OK] Cleaned ${affectedUserIds.length} normalized user subscription records`);
    ...
    if (client) {
      try {
        ...
        console.warn('[MIGRATION][WARN] Error closing database connection:', endError.message);
      }
    }
  }
