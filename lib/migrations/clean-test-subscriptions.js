...
export async function cleanTestSubscriptionsInProduction() {
  ...
  try {
    ...
    if (!hasBilling || !hasSubscriptions) {
      structuredConsole.warn('[MIGRATION][WARN] Normalized billing tables missing; skipping automatic cleanup');
      return;
    }
    ...
    if (!affectedUserIds.length) {
      structuredConsole.warn('[MIGRATION][WARN] Test subscriptions detected but no valid user_ids were resolved; skipping automated cleanup.');
      return;
    }
    ...
    structuredConsole.log(`[MIGRATION][OK] Cleaned ${affectedUserIds.length} normalized user subscription records`);
    ...
    if (client) {
      try {
        ...
        structuredConsole.warn('[MIGRATION][WARN] Error closing database connection:', endError.message);
      }
    }
  }
