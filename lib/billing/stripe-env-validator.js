...
export class StripeEnvironmentValidator {
	static validate() {
		...
		const logLevel = (
			process.env.STRIPE_VALIDATOR_LOG_LEVEL || (nodeEnv === 'production' ? 'info' : 'error')
		).toLowerCase();
		...
		if (!isProduction && (isSecretLive || isPublishableLive)) {
			warnings.push(
				'[STRIPE-VALIDATOR][WARN]  DEVELOPMENT ENVIRONMENT USING LIVE KEYS: This will charge real money'
			);
		}
		...
		if (!(isSecretTest || isSecretLive)) {
			errors.push(
				'[STRIPE-VALIDATOR][FAIL] INVALID SECRET KEY: Must start with sk_test_ or sk_live_'
			);
		}

		if (!(isPublishableTest || isPublishableLive)) {
			errors.push(
				'[STRIPE-VALIDATOR][FAIL] INVALID PUBLISHABLE KEY: Must start with pk_test_ or pk_live_'
			);
		}
		...
		if (errors.length > 0) {
			...
			if (isProduction) {
				throw new Error(
					'STRIPE ENVIRONMENT VALIDATION FAILED - Cannot start app with invalid Stripe configuration'
				);
			}
		}
		...
		if (warnings.length > 0 && logWarn) {
			...
			warnings.forEach((warning) => {
				structuredConsole.warn(`   ${warning}`);
			});
		}
		...
		if (errors.length === 0 && warnings.length === 0 && logInfo) {
			...
			structuredConsole.log(
				`[STRIPE-VALIDATOR][OK] Stripe environment validation passed for ${envType} environment`
			);
		}
	...
	static async validateSubscriptionAccess(subscriptionId) {
		...
		const validation = StripeEnvironmentValidator.validate();
		...
		if (isTestSub && isLiveEnvironment) {
			...
			structuredConsole.error(
				`   Environment: ${validation.environment.secretKeyType.toUpperCase()}`
			);
			...
			await StripeEnvironmentValidator.autoCleanTestSubscription(subscriptionId);
	...
	static async autoCleanTestSubscription(subscriptionId) {
		try {
			...
			structuredConsole.error(
				'[STRIPE-VALIDATOR][FAIL] [AUTO-CLEAN] Failed to clean subscription:',
				error.message
			);
		}
	}
