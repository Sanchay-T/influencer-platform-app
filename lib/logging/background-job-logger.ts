...
export class BackgroundJobLogger {
   ...
   */
  public updateJobState(jobId: string, state: JobState, context?: Partial<JobContext>, category?: LogCategory): void {
  ...
  public updateProgress(jobId: string, progress: JobProgress, context?: Partial<JobContext>): void {
    ...
    if (jobTimer && (now - jobTimer.startTime) > this.SLOW_JOB_THRESHOLD_MS && progress.percentage < 50) {
   ...
   */
  public completeJob(jobId: string, metrics?: JobPerformanceMetrics, context?: Partial<JobContext>): void {
    const jobTimer = this.jobTimers.get(jobId);
   ...
   */
  public failJob(jobId: string, error: Error, context?: Partial<JobContext>, retryable: boolean = true): void {
    const jobTimer = this.jobTimers.get(jobId);
    const duration = jobTimer ? Date.now() - jobTimer.startTime : undefined;
...
export const jobLog = {
  ...
  api: (jobId: string, platform: string, endpoint: string, success: boolean, responseTime: number, context?: Partial<JobContext>) => 
    backgroundJobLogger.logApiCall(jobId, platform, endpoint, success, responseTime, context),
  
  image: (jobId: string, operation: string, originalFormat: string, targetFormat: string, success: boolean, processingTime: number, context?: Partial<JobContext>) => 
    backgroundJobLogger.logImageProcessing(jobId, operation, originalFormat, targetFormat, success, processingTime, context),
  
  email: (jobId: string, operation: 'send' | 'queue' | 'deliver' | 'bounce' | 'fail', recipient: string, template: string, success: boolean, context?: Partial<JobContext>) => 
