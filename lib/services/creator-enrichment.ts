...
const INFLUENCERS_CLUB_ENDPOINT = 'https://api-dashboard.influencers.club/public/v1/creators/enrich/handle/full/';
...
export class CreatorEnrichmentService {
  ...
  private static buildSummary(result: EnrichmentApiResponse['result']): CreatorEnrichmentSummary {
    ...
    if (typeof result.location === 'string') {
      ...
      const pieces = [result.location.city, result.location.state, result.location.country].filter(Boolean);
      if (pieces.length > 0) {
    ...
    Object.entries(result).forEach(([key, value]) => {
      ...
      if (platformData.related_platforms && typeof platformData.related_platforms === 'object') {
        Object.entries(platformData.related_platforms as Record<string, unknown>).forEach(([platformKey, handle]) => {
  ...
  private static normalizeHandleForComparison(raw: string): string {
    return this.normalizeHandle(raw).toLowerCase();
  }
  ...
  private static async findCreatorRecord(args: {
    ...
    if (args.creatorId && this.isValidUuid(args.creatorId)) {
    ...
    const handleValue = args.handle ? this.normalizeHandle(args.handle) : null;
    if (!handleValue) {
      return null;
    }
    ...
    if (platformValue) {
      ...
      for (const variant of variants) {
        ...
        const matched = platformCandidates.find((candidate) => {
          const candidateHandle = typeof candidate.handle === 'string' ? this.normalizeHandle(candidate.handle) : null;
  ...
  private static async callEnrichmentApi(args: { handle: string; platform: SupportedPlatform; includeLookalikes: boolean; emailRequired: 'preferred' | 'must_have'; }): Promise<EnrichmentApiResponse> {
    ...
    if (!response.ok) {
      ...
      throw new EnrichmentApiError('Influencers.Club enrichment request failed', response.status, errorPayload);
    }
  ...
  private static normalizeMetadata(
    ...
    const summary = this.buildSummary(payload.result);
    const displayHandle = context.creatorHandle?.trim() ? context.creatorHandle : context.requestHandle;
    const displayPlatform = context.creatorPlatform?.trim() ? context.creatorPlatform : context.requestPlatform;
  ...
  private static async persistEnrichment(
    ...
    const combinedEmails = Array.from(
      new Set(
        [...existingEmails, ...normalizedEmails].filter((value) => typeof value === 'string' && value.trim().length),
      ),
    );
    const primaryEmail =
  ...
  public static async getCachedEnrichment(creatorId: string): Promise<CreatorEnrichmentRecord | null> {
  ...
  public static async getCachedEnrichmentByHandle(platform: SupportedPlatform, handle: string): Promise<CreatorEnrichmentRecord | null> {
    const creator = await this.findCreatorRecord({ platform, handle });
    if (!creator) {
      return null;
    }
  ...
  public static async enrichCreator(options: EnrichCreatorOptions): Promise<CreatorEnrichmentResult> {
    const requestHandle = this.normalizeHandle(options.handle);
    const [userProfile, existingCreator] = await Promise.all([
      getUserProfile(options.userId),
      this.findCreatorRecord({
    ...
    if (!creator) {
      creator = await this.createPlaceholderCreator({
        handle: requestHandle,
        platform: options.platform,
        externalId: options.externalId,
    ...
    if (!creator) {
      throw new CreatorNotFoundError(options.creatorId ?? options.externalId ?? `${options.platform}:${requestHandle}`);
    }

    const limit = this.resolveLimit(userProfile.currentPlan);
    ...
    const payload = await this.callEnrichmentApi({
    ...
    const record = this.normalizeMetadata(payload, {
    ...
    await this.persistEnrichment(creator.id, record, creator.metadata);
