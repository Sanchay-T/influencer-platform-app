...
export class PlanValidator {
	 ...
	 */
	static async getUserPlanStatus(
		userId: string,
		requestId?: string
		...
		try {
			...
			try {
				...
				logger.error(
					'Plan config database lookup failed, using fallback defaults',
			...
			const hasActiveSubscription =
				userProfile.subscriptionStatus === 'active' && !!userProfile.stripeSubscriptionId;
			const hasTrialingSubscription =
				userProfile.subscriptionStatus === 'trialing' && !!userProfile.stripeSubscriptionId;
			...
			const campaignsRemaining =
				planConfig.campaignsLimit === -1
					? -1
					: Math.max(0, planConfig.campaignsLimit - campaignsUsed);
			const creatorsRemaining =
				planConfig.creatorsLimit === -1 ? -1 : Math.max(0, planConfig.creatorsLimit - creatorsUsed);
	 ...
	 */
	static async validateCampaignCreation(
		userId: string,
		requestId?: string
	): Promise<ValidationResult> {
		const logRequestId = requestId || BillingLogger.generateRequestId();

		const bypass = await PlanValidator.getPlanBypassResult('campaigns');
		if (bypass) {
			await BillingLogger.logAccess(
				'GRANTED',
		...
		const planStatus = await PlanValidator.getUserPlanStatus(userId, logRequestId);
		if (!planStatus) {
			return {
				allowed: false,
		...
		if (!planStatus.isActive) {
			...
			return {
				allowed: false,
				reason:
					planStatus.trialStatus === 'expired'
						? 'Your trial has expired. Please upgrade to continue.'
						: 'Please upgrade to create campaigns.',
				upgradeRequired: true,
				recommendedPlan: PlanValidator.getRecommendedPlan('campaigns', 1),
			};
		}
		...
		if (
			planStatus.planConfig.campaignsLimit > 0 &&
			planStatus.campaignsUsed >= planStatus.planConfig.campaignsLimit
		) {
			await BillingLogger.logUsage(
				'LIMIT_EXCEEDED',
			...
			return {
				...
				usagePercentage: 100,
				upgradeRequired: true,
				recommendedPlan: PlanValidator.getRecommendedPlan(
					'campaigns',
					planStatus.campaignsUsed + 1
				),
			};
		}
		...
		const usagePercentage =
			planStatus.planConfig.campaignsLimit === -1
				? 0
	...
	static async validateCreatorSearch(
		...
		const bypass = await PlanValidator.getPlanBypassResult('creators');
		if (bypass) {
			await BillingLogger.logAccess(
				'GRANTED',
		...
		if (!planStatus.isActive) {
			...
			return {
				...
				reason:
					planStatus.trialStatus === 'expired'
						? 'Your trial has expired. Please upgrade to continue.'
						: 'Please upgrade to search for creators.',
				upgradeRequired: true,
				recommendedPlan: PlanValidator.getRecommendedPlan('creators', estimatedResults),
			};
		}
		...
		if (planStatus.planConfig.creatorsLimit > 0) {
			...
			if (projectedUsage > planStatus.planConfig.creatorsLimit) {
				...
				return {
					...
					upgradeRequired: true,
					recommendedPlan: PlanValidator.getRecommendedPlan('creators', projectedUsage),
				};
			}
		}
		...
		// Calculate usage percentage
		const usagePercentage =
			planStatus.planConfig.creatorsLimit === -1
				? 0
				: (planStatus.creatorsUsed / planStatus.planConfig.creatorsLimit) * 100;
	...
	static async validateFeatureAccess(
		...
		const allowed =
			feature === 'analytics' ? hasFeature === 'basic' || hasFeature === 'advanced' : !!hasFeature;
		...
		if (!allowed) {
			return {
				...
				upgradeRequired: true,
				recommendedPlan: PlanValidator.getRecommendedPlanForFeature(feature),
			};
		}
	...
	private static async getPlanBypassResult(
		scope: 'campaigns' | 'creators'
	 ...
	 */
	private static getRecommendedPlan(
