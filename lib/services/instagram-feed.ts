...
function keywordMatches(text: string | undefined, keyword: string): string[] {
  ...
  const seed = [keyword.toLowerCase(), "nutrition", "diet", "dietitian", "meal prep", "healthy", "wellness"];
...
function computeScore(input: {
  ...
  return playsWeight * 0.35 + engageWeight * 0.25 + shareWeight * 0.1 + recencyWeight * 0.2 + upliftWeight * 0.1 * keywordWeight;
}
...
export async function buildInstagramFeed(keyword: string): Promise<FeedRunResult> {
  ...
  const postDetailMap = new Map<string, { details: (typeof enrichedPosts)[number]['details']; transcript: (typeof enrichedPosts)[number]['transcript'] }>();
  ...
  for (const candidate of trimmed) {
    ...
    const metrics = {
      ...
      comments: postExtra.details.metrics?.comment_count ?? candidate.post.engagement?.comments ?? 0,
