...
export class BillingService {
	...
	static async getBillingStateWithCache(
		...
		const isTrialing =
			userProfile.trialStatus === 'active' && userProfile.subscriptionStatus !== 'active';
		const rawPlan = userProfile.currentPlan as PlanKey | null;
		const effectivePlan =
			isTrialing && userProfile.intendedPlan
	...
	static async getBillingState(userId: string): Promise<BillingState> {
		const { state } = await BillingService.getBillingStateWithCache(userId);
		return state;
	}
	...
	static async immediateUpgrade(
		...
		// Get current state
		const currentState = await BillingService.getBillingState(userId);
		...
		// Log the upgrade
		await BillingService.logBillingEvent(userId, 'plan_upgraded', {
		...
		// Return fresh state
		const finalState = await BillingService.getBillingState(userId);
	...
	static async reconcileWithStripe(userId: string): Promise<{
		...
		if (activeSubscription) {
			...
			stripePlan = BillingService.mapStripePriceIdToPlan(priceId);
			stripeStatus = activeSubscription.status as SubscriptionStatus;
		}

		// Compare states
		const needsUpdate =
			currentState.currentPlan !== stripePlan || currentState.subscriptionStatus !== stripeStatus;
		...
		if (needsUpdate) {
			...
			await BillingService.logBillingEvent(userId, 'state_reconciled', {
				changes,
