...
export class FeatureGateService {
  ...
  static mergeFeatures(planFeatures?: any, userOverrides?: any): FeatureMap {
    ...
    const overrides: FeatureMap = (userOverrides && typeof userOverrides === 'object') ? userOverrides : {};
  ...
  static async getFeatures(userId: string): Promise<{ currentPlan: string; features: FeatureMap } | null> {
    ...
    const planKey = profile.currentPlan || 'glow_up';
    const plan = await db.query.subscriptionPlans.findFirst({ where: eq(subscriptionPlans.planKey, planKey) });
    const features = this.mergeFeatures(plan?.features, profile.planFeatures);
    return { currentPlan: planKey, features };
  }

  static async hasFeature(userId: string, key: string): Promise<{ allowed: boolean; currentPlan: string } | null> {
    const info = await this.getFeatures(userId);
    if (!info) return null;
  ...
  static async assertExportFormat(userId: string, format: 'CSV' | 'Excel' | 'JSON') {
    const info = await this.getFeatures(userId);
    if (!info) return { allowed: false, reason: 'No profile or plan found', currentPlan: 'unknown' };
