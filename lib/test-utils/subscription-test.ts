...
export class SubscriptionTestUtils {
	 ...
	 */
	static async switchUserPlan(
		userId: string,
		newPlan: 'glow_up' | 'viral_surge' | 'fame_flex'
	): Promise<void> {
		const limits = {
	 ...
	 */
	static async simulateCampaignCreation(
		userId: string,
		count: number = 1
		...
		for (let i = 0; i < count; i++) {
			try {
				...
				structuredConsole.log(
					`✅ [TEST-UTILS] Created campaign ${i + 1}/${count} for user ${userId}`
				);
			} catch (error) {
				failed++;
				errors.push(
					`Campaign ${i + 1}: ${error instanceof Error ? error.message : 'Unknown error'}`
				);
			}
		}
	 ...
	 */
	static async simulateCreatorUsage(
		userId: string,
		creatorCount: number
		...
		try {
			...
			await PlanEnforcementService.trackCreatorsFound(
				userId,
				validation.adjustedLimit || creatorCount
			);

			structuredConsole.log(
				`✅ [TEST-UTILS] Tracked ${validation.adjustedLimit || creatorCount} creators for user ${userId}`
			);
			return { success: true };
		} catch (error) {
	...
	static async cleanupTestData(): Promise<void> {
		try {
			...
			const testCampaigns = await db.query.campaigns.findMany({
				where: (campaigns, { or, like }) =>
					or(like(campaigns.userId, 'test_%'), like(campaigns.name, 'Test Campaign%')),
			});
	...
	static async runTestSuite(): Promise<{
		...
		try {
			...
			await SubscriptionTestUtils.resetUserUsage('test_glow_up_user');
			const glowUpCampaigns = await SubscriptionTestUtils.simulateCampaignCreation(
				'test_glow_up_user',
				4
			);
			results.push({
			...
			await SubscriptionTestUtils.resetUserUsage('test_glow_up_user');
			const glowUpCreators1000 = await SubscriptionTestUtils.simulateCreatorUsage(
				'test_glow_up_user',
				1000
			);
			const glowUpCreators1001 = await SubscriptionTestUtils.simulateCreatorUsage(
				'test_glow_up_user',
				1
			);
			results.push({
			...
			await SubscriptionTestUtils.resetUserUsage('test_viral_surge_user');
			const viralSurgeCampaigns = await SubscriptionTestUtils.simulateCampaignCreation(
				'test_viral_surge_user',
				11
			);
			results.push({
			...
			await SubscriptionTestUtils.resetUserUsage('test_fame_flex_user');
			const fameFlexCampaigns = await SubscriptionTestUtils.simulateCampaignCreation(
				'test_fame_flex_user',
				15
			);
			const fameFlexCreators = await SubscriptionTestUtils.simulateCreatorUsage(
				'test_fame_flex_user',
				15000
			);
			results.push({
			...
			structuredConsole.log(
				`✅ [TEST-UTILS] Test suite completed: ${passed}/${results.length} passed`
			);
