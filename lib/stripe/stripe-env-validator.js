...
export class StripeEnvironmentValidator {
  static validate() {
    ...
    // Report results
    if (errors.length > 0) {
      console.error('[STRIPE-VALIDATOR][CRITICAL] CRITICAL ERRORS:');
      errors.forEach(error => console.error('   ' + error));
      console.error('');
      console.error('[STRIPE-VALIDATOR][ACTION] [STRIPE-VALIDATOR] TO FIX:');
    ...
    if (warnings.length > 0) {
      console.warn('[STRIPE-VALIDATOR] WARNINGS:');
      warnings.forEach(warning => console.warn('   ' + warning));
    }
    
    if (errors.length === 0 && warnings.length === 0) {
      const envType = vercelEnv === 'production' ? 'production' : vercelEnv === 'preview' ? 'preview/staging' : 'development';
      console.log(`[STRIPE-VALIDATOR][OK] Stripe environment validation passed for ${envType} environment`);
    }
  ...
  static async validateSubscriptionAccess(subscriptionId) {
    ...
    if (isTestSub && isLiveEnvironment) {
      console.error('[STRIPE-VALIDATOR][CRITICAL] SUBSCRIPTION MISMATCH DETECTED:');
      ...
      console.log('[STRIPE-VALIDATOR] CLEANUP Auto-cleaning test subscription...');
  ...
  static async autoCleanTestSubscription(subscriptionId) {
    try {
      ...
      console.error('[STRIPE-VALIDATOR][FAIL] [AUTO-CLEAN] Failed to clean subscription:', error.message);
    }
  }
}
