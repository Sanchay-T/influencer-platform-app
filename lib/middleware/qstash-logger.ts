...
export class QStashLogger {
	...
	public completeJobProcessing(
		...
		if (status === 'error') {
			...
			logger.info(
				'QStash job processing completed',
				{
					requestId: context.requestId,
					jobId,
					platform: context.platform,
			...
			logger.info(
				'QStash job processing iteration completed',
				{
					requestId: context.requestId,
					jobId,
					platform: context.platform,
	...
	public logPlatformPhase(
		...
		logger.info(
			`${platformContext.platform} processing phase: ${phase}`,
			{
				...
				batchInfo:
					platformContext.batchIndex !== undefined
						? {
				...
				progressDetails: details
					? {
							resultsProgress: details.targetResults
								? `${details.processedResults || 0}/${details.targetResults}`
								: undefined,
							apiProgress: details.maxApiCalls
								? `${details.apiCallsUsed || 0}/${details.maxApiCalls}`
								: undefined,
						}
					: undefined,
			},
		...
		logger.info(
			'QStash job statistics',
			{
				...
				performance: {
					...
					apiCallRate:
						statistics.apiCallsMade > 0
							? Math.round(statistics.totalExecutionTime / statistics.apiCallsMade)
							: 0,
				},
				processing: {
				...
				quality: {
					cacheHitRate: statistics.cacheHitRate
						? `${Math.round(statistics.cacheHitRate * 100)}%`
						: undefined,
					qualityFilterRate: statistics.qualityFilterRate
						? `${Math.round(statistics.qualityFilterRate * 100)}%`
						: undefined,
				},
			},
