...
export class ApiLogger {
  ...
  public logPhase(
    ...
    logger.debug(`API phase completed: ${phase}`, {
      ...context,
      requestId,
      phaseTime,
      totalTime: phaseTime
  ...
  public completeRequest(
    ...
    } else if (timing.duration > 5000) { // Slow request threshold
    ...
    if (Object.keys(timing.phases).length > 1) {
      logger.debug('API request performance breakdown', {
        requestId,
  ...
  public async logExternalApiCall<T>(
    ...
    logger.info(`External API call started: ${operation}`, {
      ...context,
      requestId,
      operation
    }, category);
    ...
    try {
      ...
      logger.info(`External API call completed: ${operation}`, {
        ...context,
        requestId,
        operation,
      ...
      logger.error(`External API call failed: ${operation}`, error as Error, {
        ...context,
        requestId,
        operation,
        executionTime: duration,
  ...
  public async logDatabaseOperation<T>(
    ...
    try {
      ...
      // Log slow queries
      if (duration > 1000) {
        logger.warn(`Slow database operation: ${operation}`, {
          ...context,
          requestId,
          operation,
          executionTime: duration
