...
import { YouTubeChannelProfile, YouTubeVideo, YouTubeSimilarChannel } from './types';
...
function getCategorySearchTerms(category: string): string[] {
  const categoryTerms: Record<string, string[]> = {
    ...
    entertainment: ['entertainment channels', 'challenge videos', 'viral YouTubers', 'experiment channels'],
    education: ['educational channels', 'learning videos', 'tutorial channels', 'education YouTubers'],
...
function extractTopicsFromDescription(description: string): string[] {
  ...
  const contentTypes = [
    'challenges', 'experiments', 'reviews', 'tutorials', 'vlogs',
    'unboxing', 'reactions', 'compilations', 'documentaries', 'podcasts'
  ];
 ...
 */
export function extractChannelsFromVideos(videos: YouTubeVideo[], excludeHandle: string): Array<{
  id: string;
  name: string;
  handle: string;
  thumbnail: string;
...
export function calculateSimilarityScore(
  ...
  const nameScore = calculateNameSimilarity(candidateChannel.name, targetProfile.name, searchKeywords);
 ...
 */
function calculateNameSimilarity(candidateName: string, targetName: string, keywords: string[]): number {
