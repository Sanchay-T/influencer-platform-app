...
export function transformInstagramProfile(
	...
	structuredConsole.log(
		'📊 [INSTAGRAM-TRANSFORM] Related profiles count:',
		profileData.relatedProfiles?.length || 0
	);
...
function resolveFollowerCount(profile: ApifyRelatedProfile): number | null {
	const candidateValues = [profile.followers, profile.followers_count, profile.follower_count];
...
export function transformEnhancedProfile(
	...
	const followerCount =
		typeof enhancedData.followersCount === 'number' && Number.isFinite(enhancedData.followersCount)
 ...
 */
export function filterAndDeduplicateCreators(
	creators: InstagramSimilarCreator[]
): InstagramSimilarCreator[] {
