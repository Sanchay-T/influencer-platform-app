...
import { Activity as ActivityIcon, Loader2, Play, RefreshCw, Search } from 'lucide-react';
...
const countCreatorsFromResults = (
...
const flattenCreatorsFromResults = (
	results: Array<{ creators?: PlatformResult }> | undefined
): unknown[] => {
...
export default function ClientCampaignPage({ campaign }: ClientCampaignPageProps) {
			...
			try {
				...
				if (SHOW_DIAGNOSTICS && data) {
					...
					const handlesSummary = data?.queue
						? {
								totalHandles:
									typeof data.queue.totalHandles === 'number' ? data.queue.totalHandles : undefined,
								completedHandles: Array.isArray(data.queue.completedHandles)
									? data.queue.completedHandles
									: [],
								remainingHandles: Array.isArray(data.queue.remainingHandles)
									? data.queue.remainingHandles
									: [],
							}
						: null;
					...
					setDiagnostics((prev) => ({
						...
						[current.id]: {
							...
							totalMs,
							apiCalls:
								data?.benchmark?.apiCalls ?? (Array.isArray(batches) ? batches.length : null),
							...
							<details className="mt-2">
								...
								<ul className="mt-1 space-y-1">
									{selectedDiagnostics.batches.map((batch, index) => (
										<li
											key={`${selectedJob?.id}-batch-${index}`}
						...
						<div className="space-y-2 rounded-md border border-zinc-800/60 bg-zinc-900/70 p-3 text-xs text-zinc-300">
							...
							{selectedJob.handleQueue.completedHandles.length > 0 && (
								<div>
									...
									<div className="mt-1 flex flex-wrap gap-2">
										{selectedJob.handleQueue.completedHandles.slice(-4).map((handle) => (
											<Badge
							...
							{selectedJob.handleQueue.remainingHandles.length > 0 && (
								<div>
									...
									<div className="mt-1 flex flex-wrap gap-2">
										{selectedJob.handleQueue.remainingHandles.slice(0, 4).map((handle) => (
											<Badge
												key={`upcoming-${handle}`}
												variant="outline"
	...
	const renderResults = () => {
		...
		const remainingCreators = Math.max(
			0,
			(selectedJob.totalCreators ?? 0) - processedCreators.length
		);
		...
		<div className="space-y-6">
			...
			<div className="grid gap-6 lg:grid-cols-[minmax(260px,300px),1fr]">
				...
				<div className="space-y-4 min-w-0">
					...
					{activeTab === 'creators' ? (
						<div className="space-y-4 min-w-0 relative">{renderResults()}</div>
					) : (
						<div className="space-y-4">
							{renderRunSummary()}
							{renderActivityLog()}
