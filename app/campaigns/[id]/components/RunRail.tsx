 ...
 *
 * @why Uses React.memo on job cards to prevent flickering during polling
 * Without memoization, every progress update causes all cards to re-render
 */
...
/**
 * Memoized run card to prevent flickering during polling updates
 * Only re-renders when its specific props change
 *
 * @why Custom comparison function prevents re-render when job object
 * reference changes but actual display values are the same
 */
...
}, areRunCardPropsEqual);

/**
 * Custom comparison for RunCard memo
 * Only re-render when display-relevant properties change
 */
function areRunCardPropsEqual(prev: RunCardProps, next: RunCardProps): boolean {
	return (
		prev.job.id === next.job.id &&
		prev.job.status === next.job.status &&
		prev.job.progress === next.job.progress &&
		prev.job.totalCreators === next.job.totalCreators &&
		prev.runLabel === next.runLabel &&
		prev.isSelected === next.isSelected &&
		prev.isLoadingJob === next.isLoadingJob
	);
}
...
export function RunRail({
	...
	const createSelectHandler = useCallback(
		(jobId: string) => () => onSelectJob(jobId),
		[onSelectJob]
	);
				...
				{sortedJobs.map((job, index) => {
					...
					return (
						<RunCard
							...
							onSelect={createSelectHandler(job.id)}
						/>
					);
				})}
			</CardContent>
