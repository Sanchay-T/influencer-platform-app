	...
	useEffect(() => {
		...
		if (campaign) {
			structuredConsole.log(
				'📋 [SIMILAR-SEARCH-PAGE] Campaign found in session storage:',
				campaign.id
			);
			setSearchData((prev) => ({
				...prev,
			...
			try {
				...
				if (urlCampaignId) {
					structuredConsole.log(
						'📋 [SIMILAR-SEARCH-PAGE] Campaign ID found in URL:',
						urlCampaignId
					);
					setSearchData((prev) => ({
						...prev,
				...
				<div className="flex items-center justify-between mt-2">
					<div>
						...
						<p className="text-sm text-zinc-400 mt-1">
							Search similar creators by username across platforms
						</p>
					</div>
				</div>

				<SimilarSearchForm campaignId={searchData.campaignId} onSuccess={handleSearchSubmit} />
			</div>
		</DashboardLayout>
	);
}
