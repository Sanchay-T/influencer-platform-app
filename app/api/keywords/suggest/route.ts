...
function buildCacheKey(seed: string, platform: string | undefined, planHint: string | undefined): string {
...
async function streamOpenAICompletion({
  ...
  const messages = [
    ...
    },
    {
      ...
      content: [
        `Seed keyword: "${seed}".`,
        existingKeywords.length ? `Already selected keywords: ${existingKeywords.join(', ')}.` : 'No existing keywords have been selected.',
        platform ? `Preferred platform: ${platform}.` : '',
        planHint ? `Plan tier: ${planHint}.` : '',
        anchorTokens.length ? `Anchor tokens that must remain visible (or be replaced with tight synonyms): ${anchorTokens.join(', ')}.` : '',
  ...
  try {
    const parsed = JSON.parse(content) as { suggestions?: Array<{ keyword?: string; confidence?: number; rationale?: string }> };
...
export async function POST(request: Request) {
  ...
  if (!seed) {
    return NextResponse.json({ error: 'Seed keyword is required and must be at least 3 characters' }, { status: 400 });
  }
  ...
  const limit = Math.max(1, Math.min(Number.isFinite(body?.limit) ? Number(body.limit) : 8, MAX_SUGGESTIONS));
  const platform = typeof body?.platform === 'string' ? body.platform : undefined;
  const planHint = typeof (request.headers.get('x-plan-tier') ?? '') === 'string' ? request.headers.get('x-plan-tier') ?? undefined : undefined;
