...
import { apiTracker, campaignTracker, SentryLogger, sessionTracker } from '@/lib/sentry';
...
export async function POST(req: Request) {
	return apiTracker.trackRoute('campaign', 'create', async () => {
		try {
			...
			SentryLogger.setContext('campaign_create', { userId });
			...
			if (!validation.allowed) {
				// Track feature gate denial
				SentryLogger.captureMessage('Campaign creation denied - limit exceeded', 'warning', {
					tags: { feature: 'campaign', reason: 'limit_exceeded' },
					extra: { userId, reason: validation.reason },
				});
			...
			SentryLogger.captureException(error, {
				tags: { feature: 'campaign', action: 'create' },
				extra: { errorMessage },
			});
...
export async function GET(request: Request) {
	return apiTracker.trackRoute('campaign', 'list', async () => {
		...
		try {
			...
			// Set user context for Sentry
			SentryLogger.setContext('campaign_list', { userId });
			sessionTracker.setUser({ userId });
			...
			// Capture error in Sentry with context
			SentryLogger.captureException(error, {
				tags: { feature: 'campaign', action: 'list' },
				extra: { errorMessage },
			});
