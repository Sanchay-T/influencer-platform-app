...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const paymentBehavior: Stripe.SubscriptionUpdateParams.PaymentBehavior = hasDefaultPM
			? 'error_if_incomplete'
			: 'default_incomplete';
		...
		if (!paymentIntent || paymentIntent.status === 'requires_payment_method') {
			structuredConsole.log(
				`💳 [UPGRADE-DIRECT] No payment method found, creating subscription checkout with plan details`
			);
					...
					return NextResponse.json(
						{
							...
							message:
								error?.message || 'Upgrade failed, redirect to portal to update payment method',
		...
		return NextResponse.json(
			{ error: 'Upgrade failed', detail: error?.message || String(error) },
			{ status: 500 }
		);
	}
}
