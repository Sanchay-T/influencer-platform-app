...
export async function POST(request: NextRequest) {
	...
	try {
		...
		structuredConsole.log('📥 [CHECKOUT-SUCCESS] Request payload:', {
			sessionId,
			checkoutSessionId,
		});
		...
		structuredConsole.log(
			`🎯 [CHECKOUT-SUCCESS] Processing immediate upgrade for session: ${actualSessionId}`
		);
		...
		if (session.payment_status !== 'paid') {
			structuredConsole.error(
				'❌ [CHECKOUT-SUCCESS] Payment not completed:',
		...
		if (!userId) {
			structuredConsole.error(
				'❌ [CHECKOUT-SUCCESS] No user ID found in session metadata:',
				session.metadata
			);
		...
		structuredConsole.log(
			`🔍 [CHECKOUT-SUCCESS-DEBUG] Price ID "${priceId}" mapped to plan: "${newPlan}"`
		);

		if (!newPlan) {
			structuredConsole.error(
				`❌ [CHECKOUT-SUCCESS] CRITICAL: No plan mapping found for price ID: ${priceId}`
			);
			structuredConsole.error(
				`❌ [CHECKOUT-SUCCESS] Available mappings:`,
				Object.keys(planMapping)
			);
		...
		structuredConsole.log(
			`💰 [CHECKOUT-SUCCESS] Plan identified: ${newPlan} for user: ${userId}, price ID: ${priceId}`
		);

		// ★★★ THIS IS THE KEY: IMMEDIATE PLAN UPDATE ★★★
		const upgradeTestId = `CHECKOUT_UPGRADE_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
		structuredConsole.log(
			`🎯 [CHECKOUT-SUCCESS-TEST] ${upgradeTestId} - Starting immediate upgrade`
		);
		...
		try {
			...
			structuredConsole.log(
				`✅ [CHECKOUT-SUCCESS-TEST] ${upgradeTestId} - BillingService.immediateUpgrade completed successfully`
			);
			...
			structuredConsole.error(
				`❌ [CHECKOUT-SUCCESS-TEST] ${upgradeTestId} - BillingService.immediateUpgrade FAILED:`,
				upgradeError
			);
		...
		if (!newBillingState) {
			structuredConsole.error(
				`❌ [CHECKOUT-SUCCESS] CRITICAL ERROR: newBillingState is undefined after BillingService call`
			);
			return NextResponse.json(
				{
