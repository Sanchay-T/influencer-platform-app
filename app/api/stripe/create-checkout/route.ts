...
export async function POST(req: NextRequest) {
	...
	try {
		await OnboardingLogger.logAPI(
			'REQUEST-START',
			'Stripe checkout API request received',
			undefined,
			{
		...
		if (!userId) {
			await OnboardingLogger.logAPI(
				'AUTH-ERROR',
				'Stripe checkout request unauthorized - no user ID',
				undefined,
				{
					requestId,
					error: 'UNAUTHORIZED',
		...
		await OnboardingLogger.logPayment(
			...
			'Checkout data received from frontend',
			userId,
			{
				planId,
				billing,
		...
		if (!planId) {
			await OnboardingLogger.logPayment(
				'VALIDATION-ERROR',
				'Checkout validation failed - no plan ID provided',
				userId,
				{
					requestId,
		...
		await OnboardingLogger.logPayment(
			'DB-LOOKUP-START',
			'Looking up user profile for checkout',
			userId,
			{
				planId,
				billing,
		...
		await OnboardingLogger.logPayment(
			'DB-LOOKUP-SUCCESS',
			'User profile found for checkout',
			userId,
			{
		...
		try {
			await OnboardingLogger.logPayment(
				'STRIPE-PRICE-VERIFY',
				'Verifying Stripe price configuration',
				userId,
				{
					priceId,
					requestId,
			...
			await OnboardingLogger.logPayment(
				'STRIPE-PRICE-SUCCESS',
				'Stripe price verified successfully',
				userId,
				{
					priceId: price.id,
					type: price.type,
			...
			if (interval !== expected) {
				structuredConsole.error('❌ [STRIPE-PRICE] Interval mismatch', {
					interval,
					requested: billing,
					priceId,
				});
				await OnboardingLogger.logPayment(
					'STRIPE-INTERVAL-MISMATCH',
					'Requested billing does not match price interval',
					userId,
					{
						priceId,
		...
		await OnboardingLogger.logAPI(
			'REQUEST-SUCCESS',
			'Stripe checkout API request completed successfully',
			userId,
			{
				sessionId: session.id,
				planId,
		...
		await OnboardingLogger.logError(
			...
			'Stripe checkout API request failed',
			undefined,
			{
				errorMessage,
