...
export async function GET() {
	try {
		...
		if (!planStatus)
			return NextResponse.json({ error: 'Unable to determine usage' }, { status: 500 });
		...
		return NextResponse.json({
			...
			campaigns: {
				...
				limit:
					planStatus.planConfig.campaignsLimit === -1 ? null : planStatus.planConfig.campaignsLimit,
			},
			creators: {
				used: planStatus.creatorsUsed,
				remaining: planStatus.creatorsRemaining === -1 ? null : planStatus.creatorsRemaining,
				limit:
					planStatus.planConfig.creatorsLimit === -1 ? null : planStatus.planConfig.creatorsLimit,
			},
		});
	} catch (err) {
