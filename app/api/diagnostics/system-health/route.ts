  ...
  try {
    ...
    if (userProfile) {
      diagnostics.checks.userProfile = {
        ...
        inconsistentState: userProfile.currentPlan !== 'free' && 
    ...
    if (!diagnostics.checks.backgroundJobsTable.exists) criticalIssues.push('Background jobs table missing');
    if (!diagnostics.checks.eventService.importable) criticalIssues.push('Event service not importable');
    if (!diagnostics.checks.jobProcessor.importable) criticalIssues.push('Job processor not importable');
    if (diagnostics.checks.userProfile.inconsistentState) criticalIssues.push('User in inconsistent state');

    diagnostics.overallHealth = {
      status: criticalIssues.length === 0 ? 'healthy' : 'unhealthy',
      criticalIssues,
      recommendedActions: criticalIssues.length > 0 ? [
