...
export async function GET(request: Request) {
  try {
    ...
    logger.error('Failed to resolve auth context for enrichment lookup', authError instanceof Error ? authError : new Error(String(authError)));
  ...
  if (!handle || !platformRaw) {
    return NextResponse.json({ error: 'INVALID_QUERY', message: 'handle and platform query params are required.' }, { status: 400 });
  }
  ...
  try {
    ...
    logger.error('Failed to load cached enrichment by handle', error instanceof Error ? error : new Error(String(error)), {
      handle,
      platform,
    });
    return NextResponse.json({ error: 'INTERNAL_ERROR', message: 'Unexpected error loading creator enrichment.' }, { status: 500 });
  }
}
