...
export async function POST(req: NextRequest) {
  try {
    ...
    const options: InstagramV2Options = {
      maxCreators: typeof body?.options?.maxCreators === 'number' ? body.options.maxCreators : undefined,
      postsPerCreator: typeof body?.options?.postsPerCreator === 'number' ? body.options.postsPerCreator : undefined,
    };
  ...
  try {
    ...
    const job = await db.query.scrapingJobs.findFirst({
      where: (table, operators) =>
        and(eq(table.id, jobId), eq(table.userId, userId)),
      with: {
        results: {
          columns: {
            id: true,
