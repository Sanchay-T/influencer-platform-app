...
import { scrapingJobs, type JobStatus } from '@/lib/db/schema';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log('[SIMILAR-DISCOVERY] Auth check completed', { userId, elapsed: Date.now() - startTime });
    ...
    console.log('[SIMILAR-DISCOVERY] Request body parsed', { username, platform, campaignId, targetResults: rawTarget });
    ...
    console.log('[SIMILAR-DISCOVERY] Creating job...', { jobPlatform, elapsed: Date.now() - startTime });
    ...
    if (process.env.QSTASH_TOKEN) {
      ...
      console.log('[SIMILAR-DISCOVERY] Enqueueing to QStash...', { callbackUrl, elapsed: Date.now() - startTime });
