	...
	try {
		...
		try {
			...
			structuredConsole.log(
				`✅ [ADMIN-SEARCH] Found ${userResults.length} users matching "${query}" (DB query: ${queryTime}ms)`
			);
			...
			structuredConsole.log(
				`⏱️ [ADMIN-SEARCH] Total request time: ${totalTime}ms (DB: ${queryTime}ms, Processing: ${totalTime - queryTime}ms)`
			);
