...
export async function GET(request: Request) {
	...
	try {
		...
		if (category && key) {
			...
			structuredConsole.log(
				`✅ [ADMIN-CONFIG] Retrieved ${Object.keys(configs).length} configuration categories`
			);
...
export async function PUT(request: Request) {
	...
	try {
		...
		structuredConsole.log(
			`🔄 [ADMIN-CONFIG] Bulk updating ${configurations.length} configurations`
		);
		...
		structuredConsole.log(
			`✅ [ADMIN-CONFIG] Bulk update completed: ${successCount}/${configurations.length} successful`
		);
...
// Note: No explicit OPTIONS handler. Admin APIs are intended for same-origin usage.
