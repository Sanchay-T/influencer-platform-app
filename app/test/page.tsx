...
export default function TestPage() {
	...
	const [selectedPlan, setSelectedPlan] = useState<'glow_up' | 'viral_surge' | 'fame_flex'>(
		'glow_up'
	);
	...
	const testCampaignLimits = async (count: number = 5) => {
		...
		try {
			const res = await fetch(
				`/api/test/subscription?action=create-campaigns&userId=${selectedUser}&count=${count}`
			);
			const data = await res.json();
			setTestResults(data);
	...
	const testCreatorLimits = async (count: number = 1000) => {
		...
		try {
			const res = await fetch(
				`/api/test/subscription?action=use-creators&userId=${selectedUser}&count=${count}`
			);
			const data = await res.json();
			setTestResults(data);
			await refreshUserStatus();
	...
	const switchPlan = async () => {
		...
		try {
			const res = await fetch(
				`/api/test/subscription?action=switch-plan&userId=${selectedUser}&plan=${selectedPlan}`
			);
			const data = await res.json();
			alert(data.success ? data.message : data.error);
			await refreshUserStatus();
		...
		<div className="min-h-screen bg-gray-50 py-8">
			<div className="max-w-4xl mx-auto px-4">
				<div className="bg-white rounded-lg shadow-lg p-6 mb-6">
					<h1 className="text-3xl font-bold text-center mb-8">🎉 Subscription System Test Page</h1>
					...
					{status?.success ? (
						<div className="space-y-6">
							...
							<div className="border border-blue-200 bg-blue-50 rounded-lg p-4">
								<h2 className="text-lg font-semibold text-blue-800 mb-4">📋 Subscription Plans</h2>
									...
									{status.subscriptionPlans?.map((plan: any) => (
										<div key={plan.planKey} className="bg-white rounded-lg p-4 border">
											...
											<div className="text-sm space-y-1">
												<div>
													💰 <span className="font-medium">${plan.monthlyPrice / 100}/mo</span>
												</div>
												<div>
													📊{' '}
													<span className="font-medium">
														{plan.campaignsLimit === -1 ? 'Unlimited' : plan.campaignsLimit}{' '}
														campaigns
													</span>
												</div>
												<div>
													👥{' '}
													<span className="font-medium">
														{plan.creatorsLimit === -1
															? 'Unlimited'
															: plan.creatorsLimit.toLocaleString()}{' '}
														creators
													</span>
												</div>
											</div>
							...
							<div className="border border-yellow-200 bg-yellow-50 rounded-lg p-6">
										...
										<CardContent className="space-y-4">
											<Select value={selectedUser} onValueChange={setSelectedUser}>
												...
												<SelectContent>
													...
													<SelectItem value="test_viral_surge_limit">
														Viral Surge (At Limit)
													</SelectItem>
												</SelectContent>
											</Select>
											...
											<CardContent>
												<div className="space-y-2 text-sm">
													<div className="flex justify-between">
														...
														<Badge
															variant={
																userStatus.user.currentPlan === 'fame_flex'
																	? 'default'
																	: 'secondary'
															}
														>
													...
													<div className="flex justify-between">
														...
														<span
															className={
																userStatus.usage?.campaignsUsed >=
																	(userStatus.limits?.campaignsLimit || 0) &&
																userStatus.limits?.campaignsLimit !== -1
																	? 'text-red-600 font-bold'
																	: ''
															}
														>
															{userStatus.usage?.campaignsUsed || 0} /{' '}
															{userStatus.limits?.campaignsLimit === -1
																? '∞'
																: userStatus.limits?.campaignsLimit || 0}
														</span>
													...
													<div className="flex justify-between">
														...
														<span
															className={
																userStatus.usage?.creatorsUsed >=
																	(userStatus.limits?.creatorsLimit || 0) &&
																userStatus.limits?.creatorsLimit !== -1
																	? 'text-red-600 font-bold'
																	: ''
															}
														>
															{userStatus.usage?.creatorsUsed || 0} /{' '}
															{userStatus.limits?.creatorsLimit === -1
																? '∞'
																: (userStatus.limits?.creatorsLimit || 0).toLocaleString()}
														</span>
													</div>
													{userStatus.upgradeSuggestion?.shouldUpgrade && (
														<Alert>
															<AlertDescription>
																💡 Suggested:{' '}
																{userStatus.upgradeSuggestion.suggestedPlan ||
																	'Upgrade recommended'}
															</AlertDescription>
														</Alert>
													)}
								...
								<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
									...
									<Button
										onClick={() => testCreatorLimits(1000)}
										disabled={testLoading}
									...
									<Card>
										...
										<CardContent>
											<div className="space-y-4">
													...
													<div>
														...
														<div className="space-y-2">
															{testResults.results.results.map((test: any, index: number) => (
																<div
																	key={index}
																	className={`p-2 rounded border ${test.passed ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}
																>
														...
														<div className="mt-4 p-3 bg-gray-100 rounded">
															...
															{testResults.results.summary.total} tests passed
															{testResults.results.summary.failed > 0 && (
																<span className="text-red-600 ml-2">
																	({testResults.results.summary.failed} failed)
																</span>
															)}
														</div>
													</div>
												...
												{testResults.results?.success !== undefined &&
													!testResults.results.results && (
														<div
															className={`p-3 rounded ${testResults.results.success ? 'bg-green-50' : 'bg-red-50'}`}
														>
															<div className="font-medium">
							...
							{status.testUsers && status.testUsers.length > 0 && (
								<div className="border border-purple-200 bg-purple-50 rounded-lg p-4">
									<h2 className="text-lg font-semibold text-purple-800 mb-4">👤 Test Users</h2>
									<div className="space-y-2">
										{status.testUsers.map((user: any, index: number) => (
											<div key={index} className="bg-white rounded-lg p-3 border text-sm">
												<div className="grid grid-cols-2 md:grid-cols-4 gap-2">
													<div>
														<span className="font-medium">Plan:</span> {user.currentPlan}
													</div>
													<div>
														<span className="font-medium">Campaigns:</span> {user.campaignsUsed}/
														{user.campaignsLimit}
													</div>
													<div>
														<span className="font-medium">Creators:</span> {user.creatorsUsed}/
														{user.creatorsLimit}
													</div>
													<div>
							...
							<div className="border border-gray-200 bg-gray-50 rounded-lg p-4">
								<h2 className="text-lg font-semibold text-gray-800 mb-2">🚀 Ready to Test!</h2>
								<div className="text-sm space-y-2">
									<p>
										<strong>✅ Local PostgreSQL:</strong> Running with your subscription plans
									</p>
									<p>
										<strong>✅ Plan Enforcement:</strong> Campaign and creator limits active
									</p>
									<p>
										<strong>✅ Test User:</strong> Created with Glow Up plan (3 campaigns, 1000
										creators)
									</p>
									<p>
										<strong>🌐 Server:</strong>{' '}
										<code className="bg-gray-200 px-1 rounded">http://localhost:3002</code>
									</p>
								</div>
							</div>
						</div>
