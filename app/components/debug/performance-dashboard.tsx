...
import { perfMonitor, BrowserPerformance } from '@/lib/utils/performance-monitor';
...
export function PerformanceDashboard({ className = '' }: PerformanceDashboardProps) {
  ...
  const getPerformanceGrade = (avgTime: number): { grade: string; color: string; description: string } => {
  ...
  const avgResponseTime = Object.values(summaries).reduce((sum, s) => sum + s.averageTime, 0) / Object.keys(summaries).length || 0;
  const totalCacheHits = Object.values(summaries).reduce((sum, s) => sum + (s.cacheHitRate || 0) * s.totalCalls, 0);
          ...
          {Object.keys(summaries).length === 0 ? (
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-sm text-gray-500 mt-1">Navigate around the app to start collecting metrics.</p>
              </CardContent>
            </Card>
          ) : (
            <div className="grid gap-4">
              {Object.entries(summaries).map(([operation, summary]) => {
                ...
                const cacheGrade = summary.cacheHitRate !== undefined ? getCacheHitGrade(summary.cacheHitRate) : null;
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-green-600">{summary.minTime.toFixed(2)}ms</p>
                        </div>
                        <div>
                          <p className="text-gray-600">Slowest</p>
                          <p className="font-semibold text-red-600">{summary.maxTime.toFixed(2)}ms</p>
                        </div>
                        <div>
                    ...
                    <div className="border-t pt-4">
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-mono">{browserMetrics.navigation.domContentLoaded.toFixed(2)}ms</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Load Complete</span>
                          <span className="font-mono">{browserMetrics.navigation.loadComplete.toFixed(2)}ms</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Server Response</span>
                          <span className="font-mono">{browserMetrics.navigation.serverResponse.toFixed(2)}ms</span>
                        </div>
                      </div>
                    </div>
                  </>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-red-700 mt-1">
                        Average response time is {avgResponseTime.toFixed(1)}ms. Consider implementing caching or optimizing API calls.
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-yellow-700 mt-1">
                        Cache hit rate is {(overallCacheHitRate * 100).toFixed(1)}%. Increase cache duration or improve cache strategies.
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-green-700 mt-1">
                        Your app is performing excellently with fast response times and high cache hit rates!
                      </p>
                    </div>
                  </div>
                </div>
