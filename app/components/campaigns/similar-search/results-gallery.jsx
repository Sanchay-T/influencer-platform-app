...
export default function SimilarResultsGallery({
  rows,
  selectedCreators,
  onToggleSelection,
}) {
      ...
      {rows.map((row) => {
        ...
        return (
          ...
          >
            ...
            <div className={cn('relative w-full overflow-hidden bg-zinc-800/70', platformLabel === 'youtube' ? 'aspect-video' : 'aspect-[9/16]')}>
              {preview ? (
                <img
                  src={preview}
            ...
            <div className="flex flex-1 flex-col gap-3 p-4 text-sm text-zinc-300">
              ...
              {row.followerLabel ? (
                <p className="text-xs uppercase tracking-wide text-zinc-400">
                  Followers · <span className="font-semibold text-zinc-100">{row.followerLabel}</span>
                </p>
              ) : null}
              ...
              {row.emails?.length ? (
                <div className="space-y-1 text-sm">
                  {row.emails.map((email) => (
                    <a key={email} href={`mailto:${email}`} className="block truncate text-pink-400 hover:underline">
                      {email}
                    </a>
                  ))}
                </div>
