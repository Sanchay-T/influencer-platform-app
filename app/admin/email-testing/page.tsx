	...
	const searchUsers = async (query: string) => {
		...
		try {
			const response = await fetch(
				`/api/admin/email-testing/users-cached?q=${encodeURIComponent(query)}`
			);
			if (response.ok) {
			...
			<div className="mb-6">
				<div className="flex items-center justify-between">
					...
					<div className="flex gap-2">
						<Button variant="outline" onClick={() => setEmailResults([])}>
							<RefreshCw className="h-4 w-4 mr-2" />
							Clear Results
						</Button>

						<Button variant="secondary" onClick={() => setShowBulkMode(!showBulkMode)}>
				...
				<div className="lg:col-span-1">
					<Card>
						<CardHeader>
							...
							<CardDescription>Search by email address or user ID</CardDescription>
						</CardHeader>
											...
											>
												...
												<div className="flex items-center gap-2 mt-1">
													<Badge
														variant="outline"
														className={`text-xs ${
															trialStatus.color === 'green'
																? 'text-green-700 border-green-300'
																: trialStatus.color === 'red'
								...
								<div className="border-t pt-4">
									...
									<div className="mt-2 p-4 bg-blue-50 rounded-lg space-y-2">
										...
										<div className="text-sm text-gray-600 space-y-1">
											<div>
												📧 Test emails →{' '}
												{currentUser?.emailAddresses?.[0]?.emailAddress || 'Admin email'}
											</div>
					...
					<div className="space-y-6">
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>Configure and send test emails with custom delays</CardDescription>
							</CardHeader>
							<CardContent className="space-y-6">
								...
								<div className="flex gap-3">
									...
									<Button variant="secondary" disabled={!selectedUser}>
										...
										<div className="flex items-start gap-2">
											...
											<div className="text-sm text-yellow-800">
												...
												<p className="text-xs mt-1">
													Search and select a user to send test emails.
												</p>
											</div>
										</div>
									</div>
								...
								<div className="space-y-3 text-sm text-blue-800">
									...
									<div>
										...
										<ul className="list-disc list-inside space-y-1 text-xs">
											<li>
												<strong>Send Now:</strong> Uses 30-second delay for immediate testing
											</li>
											<li>
												<strong>Preview:</strong> Review email content before sending
											</li>
											<li>
												<strong>Bulk Mode:</strong> Send to multiple users at once
											</li>
										</ul>
									</div>
									...
									<div className="bg-white/50 rounded p-2 mt-3">
										<p className="text-xs italic">
											💡 <strong>Tip:</strong> Use short delays (30s-2m) for quick testing. All
											emails are sent via Resend + QStash with full delivery tracking.
										</p>
									</div>
								</div>
							</CardContent>
