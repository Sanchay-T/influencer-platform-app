...
import { Star, Zap, Crown, CircleCheckBig, Calendar, CreditCard } from 'lucide-react';
...
function SubscriptionPlanCard() {
  ...
  const isActive = billingStatus?.hasActiveSubscription || false;
  const status = isActive ? 'Active' : 
                billingStatus?.isTrialing ? 'Trial' : 
                'Inactive';
    ...
    <Card className="h-full min-h-[320px] bg-zinc-900/80 border border-zinc-700/50">
      ...
      <CardContent className="pt-0 space-y-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-zinc-400">
                {isActive ? nextBilling.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className={
          ...
          <div className="flex items-start gap-3">
            <CircleCheckBig className={`h-5 w-5 flex-shrink-0 mt-0.5 ${
              isActive 
                ? 'text-pink-400'
                : billingStatus?.isTrialing
                  ? 'text-blue-400'
