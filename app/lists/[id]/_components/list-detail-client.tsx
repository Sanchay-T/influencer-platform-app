...
import { LayoutGrid, Link2, List as ListIcon, Loader2, Star, Trash2 } from 'lucide-react';
...
type ListItem = ListDetail['items'][number];
type ColumnState = Record<string, ListItem[]>;
					...
					<div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
						<div className="w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-950/95 p-6 shadow-[0_30px_60px_rgba(0,0,0,0.45)]">
							...
							<p className="mt-2 text-sm text-zinc-400">
								...
								<span className="font-medium text-zinc-200">{detail?.list.name}</span> and all saved
								creators inside it.
							</p>
											...
											<TableBody>
												{tableItems.length === 0 ? (
													<TableRow>
														...
														>
															No creators saved yet. Add creators from search results or campaigns
														...
														return (
															<TableRow key={item.id} className="border-zinc-800/50">
																...
																<TableCell className="text-center">
																	...
																	>
																		<Star
																			className={clsx('h-3.5 w-3.5', item.pinned && 'fill-current')}
																		/>
																	</Button>
																</TableCell>
...
function topCategory(items: ListItem[]) {
...
function bucketize(items: ListItem[]): ColumnState {
...
function flattenColumnsForDetail(columns: ColumnState): ListItem[] {
...
function resolveAvatarSource(creator: ListItem['creator']) {
...
function resolveProfileUrl(creator: ListItem['creator']) {
