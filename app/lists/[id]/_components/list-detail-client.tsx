...
export default function ListDetailClient({ listId, initialDetail }: ListDetailClientProps) {
	...
	const { status: enrichmentStatusRaw, loading: enrichmentStatusLoading, refresh: refreshEnrichmentStatus } =
		useEnrichmentStatus(listId);

	const enrichmentFallback = useMemo(() => (detail ? summarizeEnrichment(detail.items) : null), [detail]);
	...
	const handleRetryFailed = useCallback(async () => {
		...
		try {
			...
			toast.success(queued > 0 ? `Retrying ${queued} failed creator${queued === 1 ? '' : 's'}…` : 'No failed creators to retry');
			setEnrichmentCollapsed(false);
			await Promise.all([refreshEnrichmentStatus(), refreshDetail()]);
		} finally {
	...
	const shouldRenderAutoEnrichment = Boolean(
		enrichmentStatus && enrichmentStatus.total > 0 && (enrichmentStatus.active > 0 || enrichmentStatus.processed > 0)
	);
...
interface HeaderCardProps {
	...
	enrichmentPill?: { label: string; tone: 'pink' | 'amber' | 'emerald'; kind: 'active' | 'failed' | 'complete' } | null;
...
function EnrichmentFilters({
	...
	return (
		<div className="flex flex-wrap items-center gap-2">
				...
				{options.map((option) => {
					...
					return (
						...
						>
							...
							<span className={clsx('text-[11px] tabular-nums', active ? 'opacity-90' : 'text-zinc-500')}>
								{option.count}
							</span>
						</button>
					);
			...
			{value !== 'all' ? (
				<Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={() => onChange('all')}>
					Clear
				</Button>
			) : null}
		</div>
