    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-10">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-sm text-muted-foreground">
          Run the Influencers Club discovery → enrich → reel details pipeline for a keyword. Results are cached per request only.
        </p>
      </header>
      ...
      {result ? (
        <section className="grid gap-4">
          {result.items.map((item) => (
            <article key={item.postId} className="rounded-lg border border-border bg-card p-4 shadow-sm">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="text-xs uppercase tracking-wide text-muted-foreground">
                    @{item.creator.username}
                  </p>
                  <h2 className="text-xl font-semibold">{item.creator.fullName ?? item.creator.username}</h2>
                  <p className="text-sm text-muted-foreground">
                    {formatDate(item.createdAt)} • {Intl.NumberFormat(undefined, { notation: 'compact' }).format(item.metrics.plays)} plays
                  </p>
                </div>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                ...
                {item.transcript ? <span>Transcript length: {item.transcript.length} chars</span> : <span>No transcript available</span>}
              ...
              {item.transcript ? (
                <details className="mt-4 rounded-md border border-border bg-muted/20 p-3 text-sm leading-relaxed text-muted-foreground">
                  <summary className="cursor-pointer text-xs font-medium uppercase tracking-wide">Transcript</summary>
