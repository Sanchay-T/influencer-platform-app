...
async function analyze() {
  // Get the 5 most recent jobs regardless of keywords
  const jobs = await db.query.scrapingJobs.findMany({
    orderBy: [desc(scrapingJobs.createdAt)],
    limit: 5,
  });
  
  console.log('=== 5 Most Recent Jobs ===\n');
  ...
  for (const job of jobs) {
    ...
    // Check for issues
    if (creatorsInDb < job.targetResults && job.status === 'completed') {
      console.log('⚠️  UNDER TARGET: wanted', job.targetResults, 'got', creatorsInDb);
      console.log('   Gap:', job.targetResults - creatorsInDb, 'missing creators');
    }
